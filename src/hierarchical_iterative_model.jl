...
mx_4 = get_map_data(map_path, use_cache=false, road_levels=Set(1:4),
					trim_to_connected_graph = true);
mx_5 = get_map_data(map_path, use_cache=false, road_levels=Set(1:5),
					trim_to_connected_graph = true);
node_data = create_distance_trees(mx_5, metric)
...
t_1 = @elapsed opt_loc_1 = location_optimization_radius(mx_3, initial_nodes, 500,
                                                        metric, ruin_random, r)
t_2 = @elapsed opt_loc_2 = location_optimization_radius(mx_4, opt_loc_1[1][end], 500,
                                                        metric, ruin_random, r*0.5)
t_3 = @elapsed opt_loc_3 = location_optimization_radius(mx_5, opt_loc_2[1][end], 500,
                                                        metric, ruin_random, r*(0.5)^2)
t = t_1 + t_2 + t_3
opt_loc = [vcat(opt_loc_1[1], opt_loc_2[1], opt_loc_3[1]),
           vcat(opt_loc_1[2], opt_loc_2[2], opt_loc_3[2])]
...
all_nodes = mx_5.v |> keys |> collect
LLA_all = [LLA(mx_5.nodes[x], center(mx_5.bounds)) for x in all_nodes]
LLA_all = [(x.lat, x.lon) for x in LLA_all]
LLA_final = [LLA(mx_5.nodes[x], center(mx_5.bounds)) for x in final_nodes]
...
states_lat_long = [LLA(mx_5.nodes[states[i]], mx_5.bounds) for i in 1:length(states)]
...
results_df = DataFrame(datetime = Dates.now(),
					   model_type = "hierarchical-iterative",
					   metric = metric,
                       p = p,
					   m = length(mx_5.v),
					   n = length(mx_5.v),
